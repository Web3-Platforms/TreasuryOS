...
export default async function EntityDetailPage({
  ...
  try {
    ...
    const walletsData = await fetchApi<{ wallets: WalletRecord[] }>(
      `wallets?entityId=${id}`,
    );
    ...
    return (
      <AppShell>
        <div style={{ padding: "2rem" }}>
          ...
          <p>
            The entity you are looking for does not exist or you do not have
            permission to view it.
          </p>
          <Link href="/entities" style={{ color: "#0d6efd" }}>
  ...
  const canRequestWallet =
    entity.status === "approved" && entity.kycStatus === "Approved";
  ...
  const showManualBypassNotice =
    canUseManualKycBypass && entity.kycStatus !== "Approved";
  const showComingSoonKyc =
    !sumsubEnabled && !entity.kycApplicantId && entity.status === "draft";
      ...
      <div style={{ padding: "2rem", maxWidth: "800px" }}>
        ...
        >
          ...
          >
            ...
            <dd style={{ margin: 0 }}>
              {new Date(entity.lastUpdatedAt).toLocaleString()}
            </dd>
            ...
            <dd style={{ margin: 0 }}>
              <span
                style={{
                  color: entity.riskLevel === "high" ? "#ff4d4f" : "inherit",
                }}
              >
        ...
        >
          ...
          {!sumsubEnabled && (
            <p style={{ marginTop: 0, marginBottom: "1rem", color: "#f0c36d" }}>
              Sumsub KYC is coming soon. Existing KYC records remain visible,
              but new submissions are disabled for now.
            </p>
          )}
          {showManualBypassNotice && (
            <p style={{ marginTop: 0, marginBottom: "1rem", color: "#8ad1ff" }}>
              Pilot manual KYC bypass is enabled for the testnet canary.
              Operators can approve this entity internally without a live Sumsub
              applicant.
            </p>
          )}
          <dl
          ...
          >
            ...
            <dd style={{ margin: 0 }}>
              {showComingSoonKyc
                ? "Coming soon"
                : entity.latestKycWebhookType || "None"}
            </dd>
                ...
                >
                  ...
                  >
                    ...
                    <span style={{ fontSize: "0.875rem", color: "#888" }}>
                      Status:{" "}
                      <strong style={{ color: "#ccc" }}>{w.status}</strong> |
                      Sync:{" "}
