    ...
    <AppShell>
      <div style={{ maxWidth: '800px' }}>
        <div style={{ marginBottom: '1.5rem' }}>
          <Link href="/transactions" style={{ color: 'var(--muted)', fontSize: '0.875rem' }}>← Back to Transaction Queue</Link>
        </div>
        ...
        <div className="page-header">
          <div>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.4rem', marginTop: '0.5rem', alignItems: 'center' }}>
              <span className={`badge ${txCase.riskLevel === 'high' ? 'badge-red' : txCase.riskLevel === 'medium' ? 'badge-amber' : 'badge-green'}`}>
                {txCase.riskLevel} risk
              </span>
              <span className="badge badge-gray">{txCase.caseStatus}</span>
              <span style={{ color: 'var(--muted)', fontSize: '0.75rem' }} className="mono">{txCase.id}</span>
            </div>
          </div>
          <TransactionReviewActions caseId={txCase.id} status={txCase.caseStatus} />
        </div>

        <div className="page-card" style={{ padding: '1.25rem', marginBottom: '1.25rem' }}>
          <h2 style={{ fontSize: '0.75rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.08em', color: 'var(--muted)', margin: '0 0 1rem 0' }}>
          ...
          <dl className="detail-grid">
            ...
            <dd className="detail-value mono">{parseFloat(txCase.amount).toLocaleString()} {txCase.asset}</dd>

            <dt className="detail-label">Entity</dt>
            <dd className="detail-value mono">
              <Link href={`/entities/${txCase.entityId}`} style={{ color: 'var(--accent)' }}>
                {txCase.entityId}
              </Link>
            </dd>
            ...
            <dd className="detail-value">{new Date(txCase.createdAt).toLocaleString()}</dd>
          </dl>
        </div>
        ...
        <div className="page-card" style={{ padding: '1.25rem' }}>
          ...
          <dl className="detail-grid">
            ...
            <dd className="detail-value">{txCase.manualReviewRequested ? 'Yes' : 'No'}</dd>

            {txCase.reviewedAt && (
              <>
                <dt className="detail-label">Reviewed At</dt>
                <dd className="detail-value">{new Date(txCase.reviewedAt).toLocaleString()}</dd>
              </>
            )}
            ...
            {txCase.notes && (
              <>
                ...
                <dd className="detail-value" style={{ whiteSpace: 'pre-wrap', color: 'var(--ink)' }}>{txCase.notes}</dd>
              </>
            )}
            ...
            {txCase.resolutionReason && (
              <>
                ...
                <dd className="detail-value" style={{ whiteSpace: 'pre-wrap', color: 'var(--ink)' }}>{txCase.resolutionReason}</dd>
              </>
            )}
          </dl>
        </div>
