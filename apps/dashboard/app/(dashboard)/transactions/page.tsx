  ...
  return (
    <AppShell>
      <div>
        ...
        <p style={{ marginTop: 0, marginBottom: '1.5rem', color: 'var(--muted)', maxWidth: '52rem', fontSize: '0.875rem' }}>
        ...
        {screeningLoadError ? (
          ...
          <div className="page-card" style={{ padding: '1.25rem', marginBottom: '1.5rem', color: 'var(--muted)', fontSize: '0.875rem' }}>
          ...
          <div className="page-card table-scroll">
            <table className="data-table">
              ...
              <tbody>
                {cases.map((txCase) => (
                  <tr key={txCase.id}>
                    <td className="mono">{txCase.transactionReference || txCase.id.split('-')[0]}</td>
                    <td className="mono">
                    ...
                    <td>
                      <span className={`badge ${txCase.caseStatus === 'approved' ? 'badge-green' : txCase.caseStatus === 'rejected' ? 'badge-red' : txCase.caseStatus === 'under_review' ? 'badge-blue' : 'badge-gray'}`}>
                        {txCase.caseStatus}
                      </span>
                    </td>
                    <td style={{ textAlign: 'right' }}>
                      <Link href={`/transactions/${txCase.id}`} style={{ color: 'var(--accent)', fontWeight: 600, fontSize: '0.8rem' }}>
                        Review →
                      </Link>
                    </td>
                  </tr>
