    ...
    return (
      <AppShell>
        <div style={{ padding: '2rem' }}>
          ...
          <p>The wallet you are looking for does not exist or you do not have permission to view it.</p>
          <Link href="/wallets" style={{ color: '#0d6efd' }}>&larr; Back to Wallets</Link>
        </div>
      </AppShell>
    );
  }
    ...
    <AppShell>
      <div style={{ padding: '2rem', maxWidth: '800px' }}>
        <div style={{ marginBottom: '2rem' }}>
          <Link href="/wallets" style={{ color: '#aaa', textDecoration: 'none' }}>&larr; Back to Wallets Queue</Link>
        </div>
        ...
        <div style={{ background: '#111', border: '1px solid #333', borderRadius: '8px', padding: '1.5rem', marginBottom: '2rem' }}>
          ...
          <dl style={{ display: 'grid', gridTemplateColumns: '1fr 2fr', gap: '1rem', margin: 0 }}>
            ...
            <dd style={{ margin: 0, fontFamily: 'monospace', wordBreak: 'break-all' }}>{wallet.walletAddress}</dd>

            <dt style={{ color: '#aaa', fontWeight: 500 }}>Created At</dt>
            <dd style={{ margin: 0 }}>{new Date(wallet.createdAt).toLocaleString()}</dd>

            {wallet.reviewedAt && (
              <>
                <dt style={{ color: '#aaa', fontWeight: 500 }}>Reviewed At</dt>
                <dd style={{ margin: 0 }}>{new Date(wallet.reviewedAt).toLocaleString()}</dd>
              </>
            )}
        ...
        <div style={{ background: '#111', border: '1px solid #333', borderRadius: '8px', padding: '1.5rem' }}>
          ...
          <dl style={{ display: 'grid', gridTemplateColumns: '1fr 2fr', gap: '1rem', margin: 0 }}>
            ...
            {wallet.syncError && (
              <>
                <dt style={{ color: '#ff4d4f', fontWeight: 500 }}>Sync Error</dt>
                <dd style={{ margin: 0, color: '#ff4d4f' }}>{wallet.syncError}</dd>
              </>
            )}
          </dl>
        </div>
